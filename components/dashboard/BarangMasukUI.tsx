...
import Sidebar from "@/components/sidebar"; // huruf kecil s, menyesuaikan gambar foldermu
import ModalTambahBarangMasuk from "./ModalTambahBarangMasuk";
import ModalEditBarangMasuk from "./ModalEditBarangMasuk"; // Import Modal Edit Baru
import { hapusBarangMasuk } from "@/app/actions/barangMasukAction"; // Import Server Action Hapus

interface BarangMasukItem {
  id: number;
  barangId: number; // Harus ada
  tanggal: string;
  kodeBarang: string;
  namaBarang: string;
  jumlah: number;
...
export default function BarangMasukUI({
  ...
  const [selectedItem, setSelectedItem] = useState<BarangMasukItem | null>(
    null,
  );
  ...
  const handleDelete = async (id: number) => {
    const confirmDelete = window.confirm(
      "Yakin ingin menghapus data ini? Stok gudang akan otomatis dikurangi.",
    );
    if (confirmDelete) {
      setIsDeleting(id);
      await hapusBarangMasuk(id);
      setIsDeleting(null);
    }
  };
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 font-sans">
      ...
      <main className="ml-64 w-full min-h-screen flex flex-col transition-all duration-300">
